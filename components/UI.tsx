...
export const Card: React.FC<{ children: React.ReactNode; title?: string; className?: string }> = ({ children, title, className = '' }) => (
  <div className={`bg-white rounded-lg border border-slate-200 shadow-sm ${className}`}>
    {title && <div className="px-4 py-3 border-b border-slate-100 font-semibold text-slate-800 text-base">{title}</div>}
...
export const Input: React.FC<React.InputHTMLAttributes<HTMLInputElement> & { label?: string }> = ({ label, className = '', ...props }) => (
  <div className="mb-2">
    {label && <label className="block text-sm font-medium text-slate-500 uppercase mb-1">{label}</label>}
    <input className={`w-full px-3 py-2 border border-slate-300 rounded-md focus:outline-none focus:border-secondary focus:ring-1 focus:ring-secondary text-sm ${className}`} {...props} />
  </div>
);
...
export const Select: React.FC<React.SelectHTMLAttributes<HTMLSelectElement> & { label?: string }> = ({ label, children, className = '', ...props }) => (
  <div className="mb-2">
    {label && <label className="block text-sm font-medium text-slate-500 uppercase mb-1">{label}</label>}
    <select className={`w-full px-3 py-2 border border-slate-300 rounded-md focus:outline-none focus:border-secondary bg-white text-sm ${className}`} {...props}>
      {children}
    </select>
  </div>
);
