...
export const Planner = () => {
  ...
  const renderCoverageRow = () => {
      ...
      return (
        <div className="flex flex-col shrink-0 z-20 sticky top-10">
          {/* Riga Principale con i pallini */}
          <div className="flex h-8 bg-white border-b border-slate-300 shadow-sm relative z-30">
              <div 
                className="w-32 md:w-48 shrink-0 bg-white border-r border-slate-300 flex items-center pl-2 md:pl-4 font-bold text-slate-500 text-[9px] uppercase sticky left-0 z-30 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)] cursor-pointer hover:bg-slate-50 transition-colors"
                onClick={(e) => { e.stopPropagation(); setIsCoverageExpanded(!isCoverageExpanded); }}
              >
                  <ShieldCheck size={12} className={`mr-2 transition-colors ${isCoverageExpanded ? 'text-blue-500' : 'text-slate-300'}`} /> 
                  Verifica Copertura
                  {isCoverageExpanded ? <ChevronUp size={12} className="ml-auto mr-2" /> : <ChevronDown size={12} className="ml-auto mr-2" />}
              </div>
              <div className="w-[40px] md:w-[60px] shrink-0 flex items-center justify-center font-bold text-[10px] text-slate-400 border-r bg-slate-50 z-30">
                  -
              </div>
              {days.map(d => {
                  const dateKey = formatDateKey(d);
                  ...
                  // Calculation logic for unified status
                  ...
                  return (
                      <div 
                          key={`cov-${dateKey}`} 
                          className={`flex-1 min-w-[44px] md:min-w-0 flex items-center justify-center border-r border-slate-200 transition-colors cursor-pointer hover:bg-slate-50
          ...
          {/* Sottorighe Dettaglio (Espandibili) */}
          {isCoverageExpanded && (
                ...
                {['M8', 'P', 'N'].map((type) => {
                    ...
                    return (
                        <div key={type} className="flex h-6 bg-slate-50/80 border-b border-slate-200 backdrop-blur-sm">
                            <div className="w-32 md:w-48 shrink-0 bg-slate-50 border-r border-slate-200 flex items-center pl-2 md:pl-6 text-[8px] font-bold text-slate-400 uppercase sticky left-0 z-30">
                                {label}
                            </div>
  ...
  return (
    <div className="flex flex-col h-full bg-white w-full overflow-hidden" onClick={clearSelection}>
      ...
      >
                              ...
                              {groupOps.map(op => {
                                  ...
                                  return (
                                      <div key={op.id} className="flex border-b border-slate-300 hover:bg-slate-50 transition-colors h-10 md:h-8">
                                          {/* Name Column */}
                                          <div 
                                              className="w-32 md:w-48 shrink-0 bg-white border-r border-slate-300 flex items-center justify-between pl-2 md:pl-4 pr-1 sticky left-0 z-20 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)] group/op-name"
                                          >
                                              <div className="flex items-center truncate cursor-pointer hover:text-blue-600 flex-1" onClick={() => setDetailsOpId(op.id)}>
                                                  ...
                                                  <span className="font-medium text-xs md:text-sm truncate text-slate-700">{op.lastName} {op.firstName}</span>
                                              </div>
          ...
          {selectedCell && cellPopupPosition && !isMatrixView && (
              ...
              >
                      ...
                      return (
                          <div className="mb-1 border-b border-slate-100 pb-2">
                              <div className="flex justify-between items-start mb-1">
                                  <div>
                                      <div className="font-bold text-slate-800 text-sm truncate w-40">{op?.lastName} {op?.firstName}</div>
                                      <div className="text-[10px] text-slate-500 uppercase font-semibold">{format(parseISO(selectedCell.date), 'dd MMMM')}</div>
                                  </div>
                                  <div className={`px-2 py-1 rounded text-xs font-black shadow-sm border border-black/5 ${getContrastColor(shiftType?.color)}`} style={{backgroundColor: shiftType?.color || '#f1f5f9'}}>
                                      {currentCode || 'OFF'}
                                  </div>
                              </div>
                              <div className="flex flex-wrap gap-2 text-[10px] text-slate-600 mt-1.5 items-center">
                              ...
                              {hasNote && (
                                  <div className="mt-1.5 text-[10px] italic text-slate-500 truncate bg-slate-50 p-1 rounded border border-slate-100">
                                      "{entry.note}"
                                  </div>
                              )}
                          </div>
                  ...
                  <div className="grid grid-cols-6 gap-1 mb-1">
                      {state.shiftTypes.map(s => (
                          <button
                              ...
                              className={`h-7 w-full rounded text-[10px] font-bold border flex items-center justify-center transition-transform active:scale-95 ${draftShift === s.code ? 'ring-2 ring-blue-500 ring-offset-1 z-10' : 'opacity-90 hover:opacity-100'} ${getContrastColor(s.color)}`}
                      ...
                      <button 
                          ...
                          className={`h-7 w-full rounded text-[10px] font-bold border bg-slate-100 text-slate-600 flex items-center justify-center hover:bg-slate-200 ${draftShift === 'OFF' ? 'ring-2 ring-blue-500 ring-offset-1 z-10' : ''}`}
                      >
                          OFF
                      </button>
                  </div>
                  ...
                  <input 
                      className="w-full text-xs border border-slate-300 rounded px-2 py-1 focus:outline-none focus:ring-1 focus:ring-blue-500 bg-slate-50 focus:bg-white transition-colors"
      ...
      <Modal isOpen={!!dragActionPrompt} onClose={() => setDragActionPrompt(null)} title="Spostamento" className="max-w-[320px]">
          {dragActionPrompt && (
              <div className="flex flex-col gap-3">
                  <div className="flex items-center justify-between text-xs text-slate-600 bg-slate-50 p-2 rounded border border-slate-200 mb-1">
                      <div className="flex-1 text-center">
                          <div className="font-bold text-slate-800 text-[10px] truncate w-20 mx-auto">{dragActionPrompt.source.name}</div>
                          <div className="text-[10px]">{format(parseISO(dragActionPrompt.source.date || ''), 'dd/MM')} <span className="font-mono font-bold ml-1">{dragActionPrompt.source.code}</span></div>
                      </div>
                      <div className="px-2 text-slate-400"><ArrowRight size={14} /></div>
                      <div className="flex-1 text-center">
                          <div className="font-bold text-slate-800 text-[10px] truncate w-20 mx-auto">{dragActionPrompt.target.name}</div>
                          <div className="text-[10px]">{format(parseISO(dragActionPrompt.target.date || ''), 'dd/MM')} <span className="font-mono font-bold ml-1">{dragActionPrompt.target.code}</span></div>
                      </div>
                  </div>
                  ...
                  <div className="grid grid-cols-3 gap-2">
                      <button onClick={() => resolveDragAction('SWAP')} className="flex flex-col items-center justify-center p-2 rounded border border-blue-200 bg-blue-50 hover:bg-blue-100 transition-colors text-blue-800">
                          ...
                          <span className="text-[10px] font-bold mt-1">Scambia</span>
                      </button>
                      <button onClick={() => resolveDragAction('COPY')} className="flex flex-col items-center justify-center p-2 rounded border border-slate-200 bg-white hover:bg-slate-50 transition-colors text-slate-700">
                          <Copy size={16} />
                          <span className="text-[10px] font-bold mt-1">Copia</span>
                      </button>
                      <button onClick={() => resolveDragAction('MOVE')} className="flex flex-col items-center justify-center p-2 rounded border border-slate-200 bg-white hover:bg-slate-50 transition-colors text-slate-700">
                          ...
                          <span className="text-[10px] font-bold mt-1">Sposta</span>
                      </button>
                  </div>
              </div>
          )}
        ...
        {selectedCell && (
          <div className="flex flex-col md:flex-row gap-6">
            ...
            <div className="flex-1">
              ...
              <div className="mt-6">
                   <label className="block text-xs font-bold text-slate-500 uppercase mb-2">Note Turno</label>
                   <textarea
            ...
            <div className="flex-1 border-l pl-0 md:pl-6 border-slate-200">
                ...
                {/* Special Events List */}
                <div className="mb-4">
                    <h4 className="font-bold text-xs text-slate-500 uppercase mb-2">Voci Speciali Attive</h4>
                    <div className="space-y-2 max-h-40 overflow-y-auto">
                        {draftSpecialEvents.length > 0 ? draftSpecialEvents.map(ev => (
                            ...
                            <div className="text-xs text-slate-400 italic text-center py-2 border border-dashed rounded">Nessuna voce speciale</div>
                        )}
                    </div>
                </div>
                ...
                <div className="bg-slate-50 p-3 rounded-lg border border-slate-200">
                    <h4 className="font-bold text-xs text-slate-600 mb-2 flex items-center gap-1"><Plus size={12}/> Aggiungi Voce</h4>
      ...
      {/* COMPACT Day Note Editor */}
      <Modal isOpen={!!editingDayNote} onClose={() => setEditingDayNote(null)} title={`Nota ${editingDayNote ? format(parseISO(editingDayNote.date), 'dd/MM') : ''}`} className="max-w-[300px]">
          <div className="space-y-3">
               <div>
                   <div className="flex gap-1 overflow-x-auto pb-1 justify-center no-scrollbar">
                       {Object.entries(NOTE_TYPES).map(([key, config]) => (
                           <button
                               ...
                               className={`flex flex-col items-center justify-center p-1.5 rounded border min-w-[36px] aspect-square transition-all ${editingDayNote?.note.type === key ? 'bg-blue-50 border-blue-400 ring-1 ring-blue-300' : 'bg-white border-slate-100 hover:bg-slate-50'}`}
                               title={config.label}
                           >
               ...
               <textarea 
                   className="w-full text-xs border border-slate-300 rounded p-2 h-16 resize-none focus:outline-none focus:ring-1 focus:ring-blue-500 bg-slate-50 focus:bg-white"
      ...
      <Modal isOpen={!!editingOperatorNote} onClose={() => setEditingOperatorNote(null)} title={`Note: ${editingOperatorNote?.name}`} className="max-w-[300px]">
          <div className="space-y-3">
               <textarea 
                   className="w-full text-xs border border-slate-300 rounded p-2 h-24 resize-none focus:outline-none focus:ring-1 focus:ring-blue-500 bg-slate-50 focus:bg-white"
