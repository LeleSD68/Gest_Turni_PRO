  ...
  const renderCell = (op: any, day: Date) => {
    ...
    return (
      <div 
        ...
        style={{ 
            backgroundColor: shouldHighlightNote ? '#fef9c3' : (isDragOver ? undefined : (violation ? '#fee2e2' : (shiftType && !isDimmedByFilter ? shiftType.color : undefined))), 
        ...
        className={`flex-1 min-w-[44px] md:min-w-0 border-r border-slate-300 border-b border-slate-300 text-xs md:text-sm flex items-center justify-center relative transition-all h-10 md:h-8 
          ...
          ${(isRowHovered || isColHovered) && !isHoveredCell && !isSelected && !shiftType && !isRedDay && !shouldHighlightNote ? 'bg-blue-100/40' : ''} 
      ...
      >
        ...
        <div className="absolute top-0 right-0 pointer-events-auto z-40">
          ...
          {/* Note Indicator with Tooltip Support */}
          {hasNote && (highlightNotes || (!isSwap && !isEntryManual && !isVariation)) && (
              <div 
                  className={`absolute top-0 right-0 ${highlightNotes ? '-mr-0.5 -mt-0.5' : ''}`}
                  onMouseEnter={(e) => {
              ...
              >
                  {highlightNotes ? (
                      <div className="bg-yellow-100 rounded-bl-md p-0.5 shadow-sm border border-yellow-200">
                          <StickyNote size={12} className="text-yellow-600 fill-yellow-200" />
                      </div>
                  ) : (
                      <div className="w-0 h-0 border-t-[6px] border-l-[6px] border-t-yellow-500 border-l-transparent" />
                  )}
              </div>
          )}
